...
${'$'}echo: "?=response"
    ...
    private fun createInlineDefaultScript(userMessage: String, memory: String? = null, systemContext: String? = null): PpeScript {
        ...
        val systemPrompt = buildString {
            ...
            appendLine("- FIRST, you MUST call the AI to generate a JSON blueprint in this exact format:")
            appendLine("  {")
            ...
            appendLine("    ]")
            appendLine("  }")
            appendLine("- The blueprint MUST be valid JSON and include ALL files needed for the project")
            ...
            appendLine("- Use the blueprint to ensure imports/exports match between files")
            ...
            appendLine()
            appendLine("**CODE COHERENCE:**")
            appendLine("- Read existing files before modifying to keep code consistent")
            appendLine("- Use ONLY imports/exports that exist in related files (from blueprint)")
